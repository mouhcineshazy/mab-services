...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'Metadata.savings' });
  const ogLocale = locale === 'fr' ? 'fr_CA' : 'en_CA';
  const altLocale = locale === 'fr' ? 'en_CA' : 'fr_CA';
  const url = `${BASE_URL}/${locale}/epargne-investissement`;
  ...
  return {
    ...
    alternates: {
      canonical: url,
      languages: {
        'fr-CA': `${BASE_URL}/fr/epargne-investissement`,
        'en-CA': `${BASE_URL}/en/epargne-investissement`,
      },
    },
    openGraph: {
      title: t('title'),
...
export default async function SavingsPage({ params }: Props) {
  ...
  const products = ['celi', 'celiapp', 'reer', 'reee', 'reei'] as const;
  const icons    = ['💎', '🏠', '🧓', '🎓', '♿'];
  ...
  return (
    <>
      <JsonLd schema={savingsServiceSchema(locale)} />
      ...
      <div className="pt-24 pb-20">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-6 mb-16">
            {products.map((key, i) => (
              <div key={key} className="card card-accent card-hover p-7 sm:p-8">
